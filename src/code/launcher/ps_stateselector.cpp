...
void PsStateSelector::render()
{
    if (visible)
    {
        ...
        GuiLauncher::renderText(0, 110, _(text), brightWhite, font30, XALIGN_CENTER, false);   // center=true, background=false
        ...
        for (int i=0;i<4;i++)
        {
            ...
            GuiLauncher::renderText(output.x+60, 270, _("Slot")+" "+to_string(i+1), brightWhite, font24, XALIGN_LEFT, false); // center=false, background=false
        }
    }
}